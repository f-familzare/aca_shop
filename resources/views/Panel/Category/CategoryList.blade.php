                ...
                <table class="table table-striped table-advance table-hover">
                    ...
                    @foreach($categories as $category)
                        <tr>
                            ...
                            <td>
{{--                                @php($subCat=\App\Model\Category::where('id','=',$category->parent_id)->first())--}}
                                <a href="#">{{$category->parent_id==0?'دسته بندی اصلی':$category->sub_category['category_title']}}</a>
{{--                                <a href="#">{{$category->parent_id==0?'دسته بندی اصلی':$subCat->category_title}}</a>--}}
                            </td>
                            <td>
                                <a href="#">
                                    <img class="task-thumb" src="{{$category->imgUrl}}" alt="{{$category->category_title}}">
                                </a>
                            </td>
